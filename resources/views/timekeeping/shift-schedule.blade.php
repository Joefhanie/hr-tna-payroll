<x-app-layout>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group" id="row-{{ $employee->id }}">
                                ...
                                <td class="whitespace-nowrap px-4 py-3 text-right text-slate-600">
                                    @php
                                        ...
                                        foreach($activeShifts as $assignment) {
                                            if ($assignment->shift) {
                                                $displayParts[] = \Carbon\Carbon::parse($assignment->shift->start_time)->format('h:i A') . ' - ' . \Carbon\Carbon::parse($assignment->shift->end_time)->format('h:i A') . ' (' . implode(', ', array_map(function($d) { return substr($d, 0, 3); }, $assignment->shift->days_of_week ?? [])) . ')';
                                            }
                                        }
                                    @endphp
                                        ...
                                        if ($emp->currentShifts) {
                                            foreach ($emp->currentShifts as $cs) {
                                                if ($cs->shift) {
                                                    ...
                                                    $displayParts[] = \Carbon\Carbon::parse($cs->shift->start_time)->format('h:i A') . ' - ' . \Carbon\Carbon::parse($cs->shift->end_time)->format('h:i A') . ' (' . implode(', ', array_map(function($d) { return substr($d, 0, 3); }, $cs->shift->days_of_week ?? [])) . ')';
                                                }
                                            }
                                        }
    ...
    <script>
        ...
        function openEditModal(employeeId, employeeName, assignmentId, startTime, endTime, breakMinutes, days) {
            const m = document.getElementById('addShiftModal');
            m.querySelector('form').reset();
