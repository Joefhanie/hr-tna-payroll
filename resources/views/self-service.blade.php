<x-app-layout>
    ...
    <form method="GET" action="{{ route('self-service') }}" class="mb-6 flex flex-wrap items-center gap-3" id="selfServiceFilters">
        <div class="relative max-w-xs min-w-[220px] flex-1 rounded-lg bg-white">
            <svg class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <input type="search" name="q" value="{{ $filters['q'] }}" placeholder="Search employees..." class="w-full rounded-lg border-0 py-2 pl-10 pr-3 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" data-auto-submit="search" />
        </div>
        <select name="type" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-1 focus:ring-blue-500" data-auto-submit="change">
            <option value="">All roles</option>
            @foreach ($requestTypes as $type)
                <option value="{{ $type }}" @selected($filters['type'] === $type)>{{ $type }}</option>
            @endforeach
        </select>
        <select name="status" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-1 focus:ring-blue-500" data-auto-submit="change">
