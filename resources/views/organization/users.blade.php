<x-app-layout>
    ...
    <div id="userFormModal" class="{{ $errors->any() ? 'flex' : 'hidden' }} fixed inset-0 z-30 items-center justify-center bg-black/40 p-4 transition-opacity" style="padding-left: var(--sidebar-width);">
        <div class="w-full max-w-md rounded-2xl border border-slate-300 bg-white shadow-[0_25px_50px_-12px_rgba(0,0,0,0.45)] ring-4 ring-black/5">
            ...
            <form method="POST" action="{{ route('organization.users.store') }}" class="p-4">
                @csrf
    ...
    @if (isset($editingUser))
        <div id="editUserModal" class="flex fixed inset-0 z-30 items-center justify-center bg-black/40 p-4 transition-opacity" style="padding-left: var(--sidebar-width);">
            <div class="w-full max-w-md rounded-2xl border border-slate-300 bg-white shadow-[0_25px_50px_-12px_rgba(0,0,0,0.45)] ring-4 ring-black/5">
                ...
                <form method="POST" action="{{ route('organization.users.update', $editingUser) }}" class="p-4" onsubmit="document.getElementById('editUserModal').classList.replace('flex', 'hidden')">
                    @csrf
                    @method('PUT')
